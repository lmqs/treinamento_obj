import { IOperacoes } from "../ioperacoes";
import { SomaE } from "../somaE";

const makeSut = (): IOperacoes => {
  return new SomaE()
}

describe('Teste da class SomaE', () => {
  
  test('Deverá retornar erro caso o parâmetro seja menor que 0', () => {
    const sut = makeSut()
    expect(()=> {sut.somatorio(0)}).toThrow('Parâmetro inválido')
  })

  test.each([
    [20, 15],
    [10, 0],
    [50, 90]
  ])("Deverá retornar a soma de todos os números múltiplos de 3 E 5 de números naturais abaixo de %p", (valor, esperado) => {
    const sut = makeSut()
    const result = sut.somatorio(valor)
    expect(result).toEqual(esperado)
  })

  test('Deverá retornar TRUE ao executar o método validacao passando no parametro o valor e', () => {
    const sut = makeSut()
    const resultado = sut.validar('e')
    expect(resultado).toEqual(true)
  })

  test('Deverá retornar FALSE ao executar o método validar passando no parametro o valor diferente E', () => {
    const sut = makeSut()
    const resultado = sut.validar('b')
    expect(resultado).toEqual(false)
  })

  test('Deverá garantir que a função somatório vai ser chamado com o parâmetro correto', () => {
    const sut = makeSut()
    const resultSpy = jest.spyOn(sut, 'somatorio')
    sut.somatorio(20)
    expect(resultSpy).toHaveBeenCalledWith(20)
 })
});